...
// Converte una data ISO (con timezone) nel formato che <input type="datetime-local"> accetta
...
export function CommunicationForm({ supplierId, initial = null, onSubmit, onCancel, loading }) {
  const [form, setForm] = useState({
    tipo: initial?.tipo || 'EMAIL',
    data: isoToDateTimeLocal(initial?.data),
    oggetto: initial?.oggetto || '',
    corpo_note: initial?.corpo_note || '',
  })
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const payload = initial ? base : { supplier_id: supplierId, ...base }
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="col-span-2"><label className={lbl}>Oggetto</label>
        ...
        <div className="col-span-2"><label className={lbl}>Note</label>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button type="submit" disabled={loading} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-700 text-white rounded text-sm font-medium">
          {loading ? 'Salvataggio...' : (initial ? 'Aggiorna' : 'Registra')}
        </button>
      </div>
    </form>
  )
